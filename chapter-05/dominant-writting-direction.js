...
function dominantWrittingDirection(text) {
  return countBy(text, (char) => characterScript(char.codePointAt(0))?.direction || null)
    .filter((script) => script.name !== null)
    .reduce((acc, script) => (acc.count <= script.count ? script : acc), { name: 'unknown', count: -1 }).name;
}

// Test cases
expect(dominantWrittingDirection('hello world')).to.equal('ltr');
expect(dominantWrittingDirection('hello العالمية')).to.equal('rtl');
expect(dominantWrittingDirection('أهلا world')).to.equal('ltr');
expect(dominantWrittingDirection('___')).to.equal('unknown');
